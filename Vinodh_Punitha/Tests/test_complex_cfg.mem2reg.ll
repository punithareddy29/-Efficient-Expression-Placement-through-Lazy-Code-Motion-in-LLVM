...
if.then3:                                         ; preds = %if.then
  ...
  %add4 = add nsw i32 %a, %b
  %add5 = add nsw i32 %add1, %add4
  br label %if.end

if.else:                                          ; preds = %if.then
  %sub = sub nsw i32 %c, %d
  %add6 = add nsw i32 %a, %b
  %sub7 = sub nsw i32 %add1, %add6
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then3
  %x.0 = phi i32 [ %mul, %if.then3 ], [ %sub, %if.else ]
...
if.then11:                                        ; preds = %if.else8
  %add12 = add nsw i32 %add, 5
  %add13 = add nsw i32 %a, %b
  %mul14 = mul nsw i32 %sub9, %add13
  br label %if.end18

if.else15:                                        ; preds = %if.else8
  %add16 = add nsw i32 %a, %b
  %mul17 = mul nsw i32 %add16, 3
  %div = sdiv i32 %sub9, 2
  br label %if.end18

if.end18:                                         ; preds = %if.else15, %if.then11
  %x.1 = phi i32 [ %add12, %if.then11 ], [ %mul17, %if.else15 ]
  %y.1 = phi i32 [ %mul14, %if.then11 ], [ %div, %if.else15 ]
  br label %if.end19

if.end19:                                         ; preds = %if.end18, %if.end
  %x.2 = phi i32 [ %x.0, %if.end ], [ %x.1, %if.end18 ]
  %y.2 = phi i32 [ %y.0, %if.end ], [ %y.1, %if.end18 ]
  %add20 = add nsw i32 %a, %b
  %add21 = add nsw i32 %x.2, %y.2
  %add22 = add nsw i32 %add21, %add20
  ret i32 %add22
}
